    ...
    function runLogicSolver() {
        ...
        function solverCycle() {
            ...
            if (errorOccurred || isGameSolved() || !actionAppliedInLastCycle) {
                 // <<< Убрали финальный рендер/пересчет отсюда, он происходит после успешного шага >>>
                isLogicSolverRunning = false;
                updateLogicSolverButtonsState();
                saveGameState();
                if (!errorOccurred) {
                 ...
                 // <<< Полный рендер в конце, если не было ошибки, для актуализации всего >>>
                 if (!errorOccurred) {
                    renderBoard();
                 }
                return;
            ...
            try {
                ...
                if (appliedSuccessfully && appliedInfo) {
                    ...
                    console.log(`(${currentMode}) Solver Step ${stepsMade}: Applied ${lastActionType}`);
                     // <<< Рендер доски Killer после успешного шага >>>
                     if (currentMode === 'killer') {
                         renderBoard(); // Перерисовываем для обновления заметок
                     }
                } else {
